...
import { CustomElement, isListNode, ListNode, type Block } from '../utils/types';
...
const List = ({ attributes, children, element }: RenderElementProps) => {
  ...
  const listStyles = listNode.format === 'ordered' ? orderedStyles : unorderedStyles;
    ...
    if (previousEntry && nextEntry) {
      ...
      if (
        ...
        if (
          ...
          (previousList as ListNode).indentLevel === (nextList as ListNode).indentLevel
        ) {
...
const handleEnterKeyOnList = (editor: Editor) => {
  ...
  if (isListEmpty) {
    ...
    Transforms.insertNodes(editor, { type: 'list-item', children: [{ type: 'text', text: '' }] } as Block<'list-item'>);
    if (currentNode) {
...
const handleConvertToList = (editor: Editor, format: Block<'list'>['format']) => {
  const convertedPath = baseHandleConvert<Block<'list-item'>>(editor, { type: 'list-item' } as Block<'list-item'>);

  if (!convertedPath) return;

  Transforms.wrapNodes(editor, { type: 'list', format, children: [] } as Block<'list'>, { at: convertedPath });
};
...
const handleTabOnList = (editor: Editor) => {
  ...
  if (!Editor.isEditor(currentList) && isListNode(currentList as Block<'paragraph'>)) {
...
const listBlocks: Pick<BlocksStore, 'list-ordered' | 'list-unordered' | 'list-item'> = {
  ...
  'list-unordered': {
    ...
    isInBlocksSelector: true,
    handleConvert: (editor) => handleConvertToList(editor, 'unordered' as never),
    handleEnterKey: handleEnterKeyOnList,
    handleBackspaceKey: handleBackspaceKeyOnList,
    handleTab: handleTabOnList,
    snippets: ['-', '*', '+'],
