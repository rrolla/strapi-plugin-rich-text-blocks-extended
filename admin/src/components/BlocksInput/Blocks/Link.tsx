...
import { CustomElement, isLinkNode, LinkNode, type Block } from '../utils/types';
...
interface CustomReactEditor extends ReactEditor {
...
const LinkContent = React.forwardRef<HTMLAnchorElement, LinkContentProps>(
  ({ link, children, attributes }, forwardedRef) => {
    ...
    const { editor } = useBlocksEditorContext('Link') as unknown as { editor: CustomReactEditor };
    const path = ReactEditor.findPath(editor, link);
    const [popoverOpen, setPopoverOpen] = React.useState(
      editor.lastInsertedLinkPath ? Path.equals(path, editor.lastInsertedLinkPath) : false
    );
    const elementText = link.children.map((child) => 'text' in child ? child.text : '').join('');
...
const Link = React.forwardRef<HTMLAnchorElement, RenderElementProps>((props, forwardedRef) => {
  if (!isLinkNode(props.element as unknown as CustomElement)) {
    return null;
  }

  return <LinkContent {...props} link={props.element as LinkNode} ref={forwardedRef} />;
});
